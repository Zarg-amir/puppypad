      document.addEventListener('DOMContentLoaded', () => {
        ...
        function validateAndShowNextQuestion(currentQuestion) {
            ...
            if (currentQuestion === 1) {
                ...
                isValid = validateDogBreeds(validationBox);
            }
        ...
        function validateDogBreeds(validationBox) {
            let isValid = true;
            const breedInputs = document.querySelectorAll('#dogBreeds .form-input');
            breedInputs.forEach((input, index) => {
                if (input.value.trim() === '') {
                    const errorMessage = document.createElement('div');
                    errorMessage.textContent = `Please enter a breed for ${dogData.names[index]}.`;
                    validationBox.appendChild(errorMessage);
                    validationBox.style.display = 'block';
                    input.classList.add('input-error');
                    isValid = false;
